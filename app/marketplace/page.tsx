    ...
    <>
      ...
      <section className="w-full h-fit border-t-zinc-800 bg-neutral-700">
        <div className="flex flex-col items-center bg-zinc-800">
          <ul className="flex gap-6 lg:max-w-[80rem] lg:mx-auto text-neutral-500 font-medium  bg-zinc-800 w-full place-content-center sm:place-content-stretch sm:justify-between sm:px-8  h-20">
            {filter.map((item, id) => (
              <FilterButton item={item} num={67} key={id} />
            ))}
          </ul>
        </div>
        <div className="lg:max-w-[80rem] lg:mx-auto pt-10 pb-20 px-8 grid grid-cols-1 md:grid-cols-2 gap-14 md:gap-x-8 md:gap-y-16 place-items-center lg:flex lg:justify-between">
          {NFTsCardData.map(
